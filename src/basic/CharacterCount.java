...
/* Classe principal (CharacterCount) que conta a quantidade de aparições de cada personagem na série Seinfeld
...
public class CharacterCount {
    ...
    public static void main(String[] args) throws Exception {
        ...
        Job j = new Job(c, "Character count");

        // Registro das classes
        j.setJarByClass(CharacterCount.class); // main
        j.setMapperClass(MapForCharacterCount.class); // mapper
        j.setReducerClass(ReduceForCharacterCount.class); // reducer
    ...
    public static class MapForCharacterCount extends Mapper<LongWritable, Text, Text, IntWritable> {
        ...
        public void map(LongWritable key, Text value, Context con)
            ...
            for (String p: palavras){
                // regex [A-Z] só letras maiúsculas
                // regex + uma ou mais ocorrências do padrão
                // regex x$ caracter x no final do padrão
                if (p.matches("[A-Z]+:$"))  {
    ...
    No nosso caso, será Text e Iterable<IntWritable>
     */
        public void reduce(Text key, Iterable<IntWritable> values, Context con)
            ...
            // também poderia ser con.write(key, new IntWritable(soma));
        }
    }
}
